...
export default function Projects() {
    ...
    if (loading) {
        return (
            <section className="py-20 text-white px-6 sm:px-10 lg:px-[150px]">
    ...
    return (
        <section className="py-16 sm:py-20 text-white px-6 sm:px-10 lg:px-[150px]">
            <div className="w-full">
                <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-8 sm:mb-10 text-center">Featured Projects.</h2>
                {projects.length === 0 ? (
                    ...
                    <div className="relative">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                            {projects.map((project) => (
                                ...
                                >
                                    {/* Content */}
                                    <div className="p-4 sm:p-6 flex flex-col gap-3 sm:gap-4 transition-colors duration-300 group-hover:bg-white/5">
                                        <h3 className="text-base sm:text-lg font-bold flex items-center gap-2">
                                            <Folder className="w-4 h-4 sm:w-5 sm:h-5" stroke="#A3A3A3" />
                                        ...
                                        <p className="text-[#A3A3A3] text-xs sm:text-sm line-clamp-3">
                                            {project.description ?? "No description available."}
                                        </p>

                                        {project.techStack && project.techStack.length > 0 && (
                                            <div className="flex flex-wrap gap-1.5 sm:gap-2 font-semibold">
                                                {project.techStack.map((tech) => (
                                                    <span
                                                        key={tech._id}
                                                        className="px-2 sm:px-3 py-1 text-xs rounded-md bg-white/10 text-white/75 flex items-center gap-1.5 sm:gap-2"
                                                    >
                                                        <span
                        ...
                        {/* Fade effect for last row - adjusted for mobile */}
            ...
            <div className="flex justify-center mt-8 sm:mt-10 items-center">
                <Link
                    href="/portfolio"
                    className="group inline-flex items-center gap-2 text-base sm:text-lg font-semibold text-white transition-colors duration-300 hover:text-purple-400"
                >
                    See more projects
                    <ArrowRight
                        className="transition-transform duration-300 group-hover:translate-x-1 mt-0.5 w-4 h-4 sm:w-5 sm:h-5"
                        size={18}
                    />
                </Link>
            </div>
